...
(function() {
  var attending_radio, col_click, export_js;

  $(document).ready(function() {
    col_click();
    attending_radio();
    $('.notice').delay(4000).fadeOut(200);
    $("#visible_options").change(function() {
      var p;
      p = 'show=' + $("#visible_options").val() + '&sort_by=' + $("#sort_by").val() + '&order_by=' + $("#order_by").val() + '&js=true';
      return $.get("/rsvps?", p).done(function(data) {
        $("#table-js").html(data);
        return col_click();
      });
    });
    if ($("input[type=radio]:checked").val() === "true") {
      return $("#party_size_js").fadeTo(0, 1);
    }
  });

  col_click = function() {
    return $('.sort_col').click(function() {
      var ob, p;
      event.preventDefault();
      event.stopPropagation();
      ob = $("#order_by").val() === 'asc' ? 'desc' : 'asc';
  ...
  attending_radio = function() {
    return $("input:radio").on("change", function() {
      if ($("input[type=radio]:checked").val() === "true") {
        return $("#party_size_js").fadeTo(500, 1);
      } else {
        $("#party_size_js").fadeTo(500, 0);
        return $("#party_size_js").css("display", "none");
      }
    });
  };

  export_js = function() {
    return $(document).bind("ajaxSuccess", "form.export-form", function(event, xhr, settings) {
...
(function() {
  $(document).ready(function() {
    return $('.notice').delay(4000).fadeOut(200);
  });
...
(function() {
  $(document).ready(function() {
    ...
    return $("a#restaurants_js").click(function(event) {
      event.preventDefault();
      event.stopPropagation();
      return $.ajax({
