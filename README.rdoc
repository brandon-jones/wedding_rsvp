...
=====RSVP admin page simple http auth that then shows a javascript sortable table. This table will export its current view to a PDF. For example you can show only guests that are going sorted by party size and then export this report.
...
simple http auth that then shows a simple table that is exportable to a pdf. 
